...
export default function AddFriendScreen() {
  ...
  const handleCopyMyLink = async () => {
    ...
    if (!link) {
      Alert.alert("Unavailable", "Your friend link is still loading.");
      return;
    }
    await Clipboard.setStringAsync(link);
    Alert.alert("Copied", "Your friend invite link is copied.");
  };
  ...
  const handleAddByLink = async () => {
    ...
    try {
      ...
      Alert.alert("Friend added", "You can now chat with this friend.");
      setLinkOrCode("");
      router.back();
    } catch (error: any) {
      const message = error?.response?.data?.error ?? error?.message ?? "Could not add friend.";
      Alert.alert("Add friend failed", String(message));
    }
  };
